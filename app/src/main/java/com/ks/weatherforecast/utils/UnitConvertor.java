...
public class UnitConvertor {
    ...
    public static String getWindDirectionString(SharedPreferences sp, Context context, WeatherForecast weather) {
        try {
            if (weather.getWind().getSpeed() != 0) {
                String pref = sp.getString("windDirectionFormat", null);
                if ("arrow".equals(pref)) {
                    return weather.getWind().getWindDirection(8).getArrow(context);
                } else if ("abbr".equals(pref)) {
                    return weather.getWind().getWindDirection().getLocalizedString(context);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static float convertTemperature(float temperature, SharedPreferences sp) {
        ...
        if (sp.getString("unit", "°C").equals("°C")) {
            temp = UnitConvertor.kelvinToCelsius(temperature);
        } else if (sp.getString("unit", "°C").equals("°F")) {
            temp = UnitConvertor.kelvinToFahrenheit(temperature);
        } else {
            temp = temperature;
        }
